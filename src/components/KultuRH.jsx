...
import { C, FF, KEYFRAMES, STEPS, calcScore } from "./tokens";
import { Nav, Stepper, Field, Spinner, Tag, ResultCard } from "./Atoms";
import { FunctionTable, EssentialPrioritization } from "./FunctionCard";
import {
    ...
    <div style={{ marginTop: 56 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 16 }}>
        {items.map(item => {
          ...
          const gc = g => ({ A: C.secondary, B: C.success, C: C.primary, D: C.warn, E: C.error }[g] || C.primary);
          return (
            <div key={item.key} style={{
              ...
              {isOpen && (
                <div style={{ borderTop: `1px solid ${C.border}`, padding: "14px 18px", background: C.surfaceAlt }}>
                  <div style={{ fontSize: 13, color: C.textSecondary, fontFamily: FF, lineHeight: 1.75, marginBottom: 12, display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {item.resumen}
                  </div>
...
/* ═══════════════ RESULTS ════════════════ */
function Results({ result, form, mode, onReset, onOpenPdf }) {
        ...
        <ResultCard title="Perfil del candidato ideal" icon="👤">
          ...
          {result.perfilIdeal.conocimientosTecnicos?.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{result.perfilIdeal.conocimientosTecnicos.map((k, i) => <Tag key={i}>{k}</Tag>)}</div>
            </div>
          )}
...
function FuncionesBloques({ funciones, onChange }) {
  const SCORE_THRESHOLD = 10; // umbral para marcar esencial
  const calcFnScore = f => (f.freq * f.impact) + f.complexity;

  const E2E_LABELS = ["Planificación", "Análisis", "Diseño / Desarrollo", "Ejecución", "Supervisión", "Comunicación", "Mejora continua"];

  const updateFn = (idx, patch) => {
    const updated = funciones.map((f, i) => i !== idx ? f : { ...f, ...patch });
    onChange("funcionesEstructuradas", updated);
    onChange("responsabilidades", updated.map(f => f.desc).join(" | "));
  };
...
function DraftRecoveryToast({ draft, onRestore, onDiscard }) {
  return (
    <div style={{
...
/* ═══════════════ MAIN APP ═══════════════ */
export default function KultuRH({ user, company, onSignOut }) {
  ...
  const [pendingDraft, setPendingDraft] = useState(null); // draft waiting for user decision
  ...
  useEffect(() => {
    try {
      ...
      // Solo restaurar si el draft tiene menos de 24 horas
      if (draft.savedAt && Date.now() - draft.savedAt < 86400000) {
        setPendingDraft(draft);
      } else {
        localStorage.removeItem(DRAFT_KEY); // draft expirado
      }
    } catch { }
  }, []);
  ...
  const restoreDraft = () => {
    ...
    if (d.formC) setFormC(prev => ({ ...prev, ...d.formC }));
    if (d.formL) setFormL(prev => ({ ...prev, ...d.formL }));
  ...
  useEffect(() => {
    const handleVisibility = () => {
      ...
      // No refrescar si pasaron menos de 60 segundos
      if (Date.now() - lastRefresh.current < 60000) return;
      lastRefresh.current = Date.now();
      // Refrescar token sin re-import
  ...
  const canNext = () => {
    if (mode === "levantar" && step === 2) {
      ...
      return formL.funciones.some(fn => (fn.raw || fn.desc || "").trim().length > 0);
    }
    if (!grp) return false;
    return grp.fields.filter(f => f.required).every(f => (form[f.key] || "").trim().length > 0);
  };

  const handleSelect = m => { setMode(m); setScreen("form"); setStep(1); setSubPhase(null); setPendingDraft(null); };
  const handleReset = () => {
    clearDraft(); // Limpiar localStorage PRIMERO
    setPendingDraft(null); // Cerrar toast si existe
  ...
  // ─── Anthropic helper con diagnóstico estructurado ───────────────────────
  ...
  const getAiErrorMsg = (err) => AI_MESSAGES[err.message] || AI_MESSAGES.DEFAULT;
  ...
  const predictFields = async () => {
    ...
    try {
      ...
      // Guardar funciones estructuradas si vienen en array
      const fnArray = Array.isArray(parsed.funciones) ? parsed.funciones : [];
      // Convertir también a texto para compatibilidad con el prompt final
      const fnTexto = fnArray.map(f => f.desc).join(" | ");
      setFormC(f => ({
        ...f, ...parsed,
  ...
  const generate = async () => {
    ...
    try {
      ...
      setScreen("result");
      clearDraft(); // Trabajo terminado — limpiar borrador
    } catch (err) {
      console.error("[KultuRH] generate:", err.message);
      // Datos del usuario están seguros — solo mostramos error y dejamos reintentar
      setError(getAiErrorMsg(err));
      setScreen("form");
    }
  };
  ...
  const handleNext = () => {
    // Si en modo "crear" paso 1 y YA hay predicciones, no re-consumir la API
  ...
  if (screen === "profile" && viewingProfile) {
    return (
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FF }} ref={topRef}>
        ...
        <Nav screen={screen} mode={mode} step={step} onReset={handleReset} company={company} onSignOut={onSignOut} userEmail={user?.email} onToggleAdmin={() => { setViewingProfile(null); setScreen("admin"); }} />
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FF }} ref={topRef}>
      <style>{KEYFRAMES}</style>
      <Nav screen={screen} mode={mode} step={step} onReset={handleReset} company={company} onSignOut={onSignOut} userEmail={user?.email} onToggleAdmin={() => { setViewingProfile(null); setScreen("admin"); setMode(null); }} />
          ...
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            ...
            <div style={{
              ...
              {mode === "levantar" && step === 2 ? (
                ...
                // Bloques confirmatorios para funciones generadas por IA en modo Crear
                <FuncionesBloques
                  funciones={formC.funcionesEstructuradas}
                  onChange={(k, v) => setFormC(f => ({ ...f, [k]: v }))}
                />
              ) : (
