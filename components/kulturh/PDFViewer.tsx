...
import { generatePdfBlob, PDF_SECTIONS } from "@/components/kulturh/ProfilePDF";
...
export default function PDFViewer({ result, form, mode, company, onClose }: PDFViewerProps) {
    const [blobUrl, setBlobUrl] = useState<string | null>(null);
    const [loading, setLoading] = useState(true);
    const [sections, setSections] = useState<string[]>([]); // empty = all
    const [error, setError] = useState<string | null>(null);

    const buildPdf = useCallback(async (selectedSections: string[]) => {
        setLoading(true);
        setError(null);
        try {
